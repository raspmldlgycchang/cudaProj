...
#define GRIDWIDTH (WIDTH/TILE_WIDTH)
...
__host__ int main(void)
{
	...
	dim3 dimBlock(WARPSIZE, WARPSIZE, 1);
	...
	printf("g_C[%4d][%4d] = %u\n", 0, 0, pC[0 * WIDTH + 0]);
	printf("g_C[%4d][%4d] = %u\n", 51, 50, pC[51 * WIDTH + 50]);
	printf("g_C[%4d][%4d] = %u\n", WIDTH-1, WIDTH-1, pC[(WIDTH-1) * WIDTH + (WIDTH-1)]);
	//시간출력
